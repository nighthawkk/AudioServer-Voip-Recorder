#!/usr/bin/env python3
# host_audioserver.py
# Receives PCM chunks from agent_audioserver.js and writes them locally
# .bc (uplink) and .ac (downlink)
...
def close_and_rename(thisPtr, ext):
    ...
    print(f"[+] Closed {thisPtr} -> {final}", flush=True)
...
def on_message(message, data):
    if message["type"] == "send":
        ...
        if "sampleRate" in payload:
            print("[agent]", "sampleRate: "+str(payload["sampleRate"]), flush=True)
        if "uid" in payload:
            uid = payload["uid"]
            print("[agent]", "uid: "+str(uid), flush=True)
            if uid not in uidMap.values():
                return
        if "log" in payload:
            print("[agent]", payload["log"], flush=True); return
        ...
        if event == "recordGetNextBuffer":
            ...
            print("[agent]","recordReleaseBuffer Called", flush=True)
            pass  # optional
        elif event == "trackReleaseBuffer":
            print("[agent]","trackReleaseBuffer Called", flush=True)
            pass  # optional
        elif event == "trackStop":
            print("[agent]","trackStop Called", flush=True)
            close_and_rename(thisPtr, ".ac")
        elif event == "recordStop":
            print("[agent]","recordStop Called", flush=True)
            close_and_rename(thisPtr, ".bc")
    elif message["type"] == "error":
        print("[agent-error]", message, flush=True)
